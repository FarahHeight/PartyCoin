...
{
    return GetBoolArg("-testnet", false) ? 21655 : 72565;
}
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop PartyCoin server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "PartyCoin server stopping";
}
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("PartyCoin-rpclist");
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("PartyCoin-rpchand");
